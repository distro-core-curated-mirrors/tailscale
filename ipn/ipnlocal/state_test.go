...
func TestServicesNotClearedByStart(t *testing.T) {
	...
	if cc.hi == nil || len(cc.hi.Services) == 0 {
		t.Fatalf("test setup bug: services should be present")
	}

	mustDo(t)(lb.Start(ipn.Options{}))

	if len(cc.hi.Services) == 0 {
		t.Errorf("services should still be present in hostinfo after no-op Start")
	}
	...
	// peerAPIListeners.
	if len(cc.hi.Services) == 0 {
		t.Errorf("services STILL not present after authreconfig")
	}
}
