...
// toLogMessageV1 converts an AUM and NetworkLockUpdate to the JSON output
// returned by the CLI.
func toLogMessageV1(aum tka.AUM, update ipnstate.NetworkLockUpdate) logMessageV1 {
	...
	if len(aum.PrevAUMHash) > 0 {
		prevAUMHash := aum.PrevAUMHash.String()
		expandedAUM.PrevAUMHash = &prevAUMHash
	}
	if key := aum.Key; key != nil {
		expandedKey := toExpandedKeyV1(key)
		expandedAUM.Key = expandedKey
	}
	if keyID := aum.KeyID; keyID != nil {
		expandedKeyID := fmt.Sprintf("tlpub:%x", keyID)
		expandedAUM.KeyID = expandedKeyID
	}
	if state := aum.State; state != nil {
		expandedState := expandedStateV1{}
		if h := state.LastAUMHash; h != nil {
			lastAUMHash := h.String()
			expandedState.LastAUMHash = lastAUMHash
		}
